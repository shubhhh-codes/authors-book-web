...
if (!SECRET) {
  throw new Error(
    'NEXTAUTH_SECRET is not set. Define it in .env.local to sign admin session tokens.'
  );
}
...
export async function signAdminSessionToken(): Promise<string> {
  ...
  const expiresAt = issuedAt + SEVEN_DAYS_MS;
...
export async function verifyAdminSessionToken(token: string | undefined | null): Promise<boolean> {
  ...
  const expectedSignature = await generateHmacSha256(payload, secretKey);
