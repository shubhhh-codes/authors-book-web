...
class ShiprocketService {
  ...
  verifyWebhookSignature(rawBody: string, signature: string): boolean {
    ...
    // Shiprocket spec uses Base64-encoded HMAC, but check both for compatibility
    ...
    const isValid = signature === base64Digest || signature === hexDigest;
    if (!isValid) {
  ...
  async generateAccessToken(
    cartItems: Array<{ variant_id: string; quantity: number; sku?: string }>,
    redirectUrl: string
